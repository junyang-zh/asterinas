...
extern "C" fn trap_handler(f: &mut TrapFrame) {
    ...
    let exception = match cause {
        Trap::Interrupt(interrupt) => {
            call_timer_soft_or_ext_irq_callbacks(f, interrupt, PrivilegeLevel::Kernel);
            return;
        }
...
pub(super) fn call_timer_soft_or_ext_irq_callbacks(
    ...
    match interrupt {
        ...
        Interrupt::SupervisorSoft => {
            let ipi_irq_num = super::irq::get_ipi_irq_num() as u8;
            call_irq_callback_functions(
                trap_frame,
