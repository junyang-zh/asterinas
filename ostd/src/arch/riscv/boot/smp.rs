...
pub(crate) unsafe fn bringup_all_aps(info_ptr: *const PerApRawInfo, pt_ptr: Paddr, num_cpus: u32) {
    ...
    // SAFETY: The caller ensures we haven't booted any APs. We ensure that the
    // variables are properly defined in the assembly code.
    unsafe {
...
///     up correctly,
///  3. and the `hart_id` hart hasn't booted.
...
/// # Safety
///
/// The caller must ensure that this function is only called once before
/// booting any APs, and the boot code correctly defines the variable
