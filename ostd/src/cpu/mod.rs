...
/// An error indicating that the CPU does not exist.
#[derive(Debug)]
pub struct NonexistentCPU(pub usize);
...
impl TryFrom<usize> for CpuId {
    type Error = NonexistentCPU;

    fn try_from(value: usize) -> Result<Self, Self::Error> {
        if value < num_cpus() {
            Ok(CpuId(value as u32))
        } else {
            Err(NonexistentCPU(value))
        }
    }
}
...
impl TryFrom<u32> for CpuId {
    ...
    fn try_from(value: u32) -> Result<Self, Self::Error> {
        if (value as usize) < num_cpus() {
            ...
            Err(NonexistentCPU(value as usize))
        }
    }
}
