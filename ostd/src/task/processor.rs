...
pub(super) fn switch_to_task(next_task: Arc<Task>) {
    ...
    drop_old_task_and_set_old_to(current_task_ptr);
    ...
    // This must be done after storing `CURRENT_TASK_PTR` since the caller
    // should see `Task::current()` as `next_task`.
    if let Some(handler) = POST_SCHEDULE_HANDLER.get() {
        // FIXME: It is not safe to unwind here, as `CURRENT_TASK_PTR` and
        // `PREVIOUS_TASK_PTR` are contaminated.
        handler();
    }
...
fn drop_old_task_and_set_old_to(current_task_ptr: *const Task) {
...
pub(super) fn on_task_entry_or_resume() {
    // Release the previous task's running flag.
    let prev = PREVIOUS_TASK_PTR.load();
    if !prev.is_null() {
        // SAFETY: The pointer is set by `switch_to_task` and is guaranteed to be
        // built with `Arc::into_raw`.
        let prev_task = unsafe { &*prev };
        prev_task.switched_on_cpu.store(false, Ordering::Release);
    }
