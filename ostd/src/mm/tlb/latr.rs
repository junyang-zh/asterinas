...
use crate::{
    cpu::{AtomicCpuSet, CpuId, CpuSet, PinCurrentCpu, all_cpus, num_cpus},
...
pub(crate) fn init_bsp() {
    ...
    crate::timer::register_callback_on_cpu(timer_flush);
}
...
pub(super) fn add_lazy_frame(
    cpu_set: &CpuSet,
    op: TlbFlushOp,
    frame: RcuDrop<Frame<dyn AnyFrameMeta>>,
) -> Result<(), RcuDrop<Frame<dyn AnyFrameMeta>>> {
...
cpu_local! {
    ...
    static FRAMES: RefCell<[Option<RcuDrop<Frame<dyn AnyFrameMeta>>>; LATR_ARRAY_SIZE]> = RefCell::new([const { None }; LATR_ARRAY_SIZE]);
