    ...
    pub fn unmap(&mut self) -> usize {
        ...
        match frag {
            ...
            PageTableFrag::StrayPageTable {
                ...
                let (pt, panic_guard) = unsafe { RcuDrop::into_inner(pt) };
                let rcu_pt = RcuDrop::new(pt);
                panic_guard.forget();
                let rcu_frame = Frame::rcu_from_unsized(rcu_pt);
                self.flusher
                    .issue_tlb_flush_with(TlbFlushOp::for_range(va..va + len), rcu_frame);
