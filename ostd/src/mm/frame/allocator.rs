...
///
/// OSTD allows customized frame allocators by the [`global_frame_allocator`]
...
pub struct GlobalFrameAllocator {
    ...
    pub alloc: fn(core::alloc::Layout) -> Option<Paddr>,
    /// Deallocates a contiguous range of frames.
    pub dealloc: fn(Paddr, usize),
}

extern "Rust" {
    /// The global frame allocator.
    static __GLOBAL_FRAME_ALLOCATOR: GlobalFrameAllocator;
}
...
fn alloc_upcall(layout: core::alloc::Layout) -> Option<Paddr> {
    ...
    unsafe { (__GLOBAL_FRAME_ALLOCATOR.alloc)(layout) }
}
...
pub(super) fn dealloc_upcall(addr: Paddr, size: usize) {
    ...
    // with the `global_frame_allocator` attribute. If they use safe code only
    // then the up-call is safe.
    unsafe { (__GLOBAL_FRAME_ALLOCATOR.dealloc)(addr, size) }
}
