...
/// Get the reference to a metadata slot.
fn get_slot(paddr: Paddr) -> Result<&'static MetaSlot, GetFrameError> {
...
impl MetaSlot {
    ...
    ///  - the metadata stored is initialized (by [`Self::write_meta`]) and valid.
    ///
    ...
    /// The returned pointer should not be dereferenced as mutable unless having
    /// exclusive access to the metadata slot.
    pub(super) unsafe fn meta_ptr_as<M: AnyFrameMeta>(&self) -> *mut M {
        // SAFETY: If the type matches `M` as the caller guarantees, then the
        // metadata is initialized and valid to be dereferenced as `M`.
        unsafe { &mut *(self.storage.get() as *mut M) }
    }
