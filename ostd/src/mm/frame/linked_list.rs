...
use super::{
    ...
    unique::{UniqueFrame, UniqueFrameRefMut},
    MetaSlot,
};
use crate::{
...
/// assert_eq!(cursor.current().unwrap().meta().mark, 2);
/// cursor.move_next();
/// assert_eq!(cursor.current().unwrap().meta().mark, 1);
/// ```
///
    ...
    Link<M>: AnyFrameMeta,
{
    ...
    pub fn cursor_mut_at(&mut self, frame: Paddr) -> Option<CursorMut<'_, M>> {
        ...
        if contains {
            Some(CursorMut {
                list: self,
                current: Some(NonNull::new(unsafe { slot.meta_ptr_as::<Link<M>>() }).unwrap()),
            })
        } else {
            None
        }
    ...
    Link<M>: AnyFrameMeta,
{
    ...
    pub fn move_next(&mut self) {
        self.current = match self.current {
            ...
            Some(current) => unsafe { *current.as_ref().next.get() },
    ...
    pub fn move_prev(&mut self) {
        self.current = match self.current {
            // SAFETY: The cursor is pointing to a valid element.
            Some(current) => unsafe { *current.as_ref().prev.get() },
            None => self.list.back,
        };
    }

    /// Gets the mutable reference to the current frame.
    pub fn current(&mut self) -> Option<UniqueFrameRefMut<'_, Link<M>>> {
        self.current.map(|current| {
            let paddr = mapping::meta_to_frame::<PagingConsts>(current.as_ptr() as Vaddr);
            // SAFETY: `&mut self` ensures we have exclusive access to the frame.
            unsafe { UniqueFrameRefMut::from_raw(paddr) }
        })
    }
    ...
    pub fn take_current(&mut self) -> Option<UniqueFrame<Link<M>>> {
        let frame = self.current.map(|current| {
        ...
        // SAFETY: We own the links with `&mut self`.
        unsafe {
            let next_ptr = *frame.meta().next.get();
            if let Some(prev) = &mut *frame.meta().prev.get() {
                debug_assert_eq!(*prev.as_mut().next.get_mut(), Some(self.current.unwrap()));
                *prev.as_mut().next.get_mut() = next_ptr;
            } else {
                self.list.front = next_ptr;
            }
            let prev_ptr = *frame.meta().prev.get();
            if let Some(next) = &mut *frame.meta().next.get() {
                debug_assert_eq!(*next.as_mut().prev.get_mut(), Some(self.current.unwrap()));
                *next.as_mut().prev.get_mut() = prev_ptr;
            ...
            *frame.meta().next.get() = None;
            *frame.meta().prev.get() = None;
        }
    ...
    pub fn insert_before(&mut self, mut frame: UniqueFrame<Link<M>>) {
        ...
        debug_assert!(frame.meta_mut().next_mut().is_none());
        debug_assert!(frame.meta_mut().prev_mut().is_none());
        ...
        if let Some(current) = &mut self.current {
            ...
            unsafe {
                if let Some(prev) = current.as_mut().prev.get_mut() {
                    debug_assert_eq!(*prev.as_mut().next.get_mut(), Some(*current));
                    *prev.as_mut().next.get_mut() = Some(frame_ptr);
                    *frame.meta_mut().prev.get_mut() = Some(*prev);
                    *frame.meta_mut().next_mut() = Some(*current);
                    *prev = frame_ptr;
                } else {
                    debug_assert_eq!(self.list.front, Some(*current));
                    *frame.meta_mut().next_mut() = Some(*current);
                    *current.as_mut().prev.get_mut() = Some(frame_ptr);
            ...
            if let Some(back) = &mut self.list.back {
                // SAFETY: We have ownership of the links via `&mut self`.
                unsafe {
                    debug_assert!(back.as_mut().next.get_mut().is_none());
                    *back.as_mut().next.get_mut() = Some(frame_ptr);
                }
                *frame.meta_mut().prev.get_mut() = Some(*back);
...
#[derive(Debug)]
pub struct Link<M> {
    // The prev and next pointers are mutably borrowed when the list is mutably
    // borrowed.
    next: UnsafeCell<Option<NonNull<Link<M>>>>,
    prev: UnsafeCell<Option<NonNull<Link<M>>>>,
    meta: M,
}
...
impl<M> Link<M> {
    ...
    fn next_mut(&mut self) -> &mut Option<NonNull<Link<M>>> {
        self.next.get_mut()
    }

    fn prev_mut(&mut self) -> &mut Option<NonNull<Link<M>>> {
        self.prev.get_mut()
    }
}
...
mod test {
    ...
    fn linked_list_push_pop() {
        ...
        cursor.move_next();
        // 2 1 *3 ()
        assert_eq!(cursor.current().unwrap().meta().mark, 3);
        cursor.move_next();
        // 2 1 3 *()
    ...
    fn linked_list_cursor_ops() {
        ...
        assert_eq!(cursor.current().unwrap().meta().mark, 3);
        cursor.move_next();
        assert_eq!(cursor.current().unwrap().meta().mark, 4);
        cursor.move_next();
        assert_eq!(cursor.current().unwrap().meta().mark, 1);
        cursor.move_next();
        assert_eq!(cursor.current().unwrap().meta().mark, 5);
        cursor.move_next();
        assert!(cursor.current().is_none());
        // 2 3 4 1 5 *()
