...
use super::{PageTableConfig, PageTableNode, node::PageTablePageMeta};
use crate::{
    cpu::PinCurrentCpu,
    cpu_local, irq,
    mm::{FrameAllocOptions, UniqueFrame},
...
pub(super) fn alloc<C: PageTableConfig>(meta: PageTablePageMeta<C>) -> PageTableNode<C> {
    let preempt_guard = disable_preempt();
    let cpu = preempt_guard.current_cpu();
    let pool_size = POOL_SIZE.get_on_cpu(cpu);
