...
impl<E: PageTableEntryTrait, C: PagingConstsTrait> PageTableNode<E, C> {
    ...
    pub(super) fn lock_write(self) -> PageTableWriteLock<E, C> {
        while self
            .meta()
            .lock
            .compare_exchange_weak(UNLOCKED, WRITE_LOCKED, Ordering::AcqRel, Ordering::Relaxed)
            .is_err()
        {
            core::hint::spin_loop();
        }
    ...
    pub(super) fn lock_read(self) -> PageTableReadLock<E, C> {
        loop {
            let lock_val = self.meta().lock.load(Ordering::Relaxed);
            if lock_val & WRITE_LOCKED != 0 {
                core::hint::spin_loop();
                continue;
            }
            if self
        ...
        PageTableReadLock::<E, C> { frame: Some(self) }
    }
...
impl<E: PageTableEntryTrait, C: PagingConstsTrait> PageTableReadLock<E, C> {
    ...
    pub(super) fn unlock(mut self) -> PageTableNode<E, C> {
        self.meta().lock.fetch_sub(READ_LOCKED, Ordering::Relaxed);
...
impl<E: PageTableEntryTrait, C: PagingConstsTrait> PageTableWriteLock<E, C> {
    ...
    pub(super) fn unlock(mut self) -> PageTableNode<E, C> {
        // Release the lock.
        self.meta().lock.store(UNLOCKED, Ordering::Release);
