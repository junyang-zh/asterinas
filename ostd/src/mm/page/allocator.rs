...
const LOCAL_CACHE_SIZE: usize = 1 * 1024 * 1024 * 1024 / PAGE_SIZE;
...
impl LocalCache {
    ...
    fn alloc_slow(&mut self) -> Paddr {
        ...
        let paddr = allocator.alloc(LOCAL_LOW_WATERMARK).unwrap() * PAGE_SIZE;
...
pub(crate) fn alloc_single<M: PageMeta>(metadata: M) -> Option<Page<M>> {
    if !crate::INITIALIZED.load(Ordering::Acquire) {
        return PAGE_ALLOCATOR.get().unwrap().lock().alloc(1).map(|idx| {
