...
/// Allocates frames with the given layout in bytes.
pub fn alloc(layout: Layout) -> Option<Paddr> {
...
/// Deallocates frames in the given range.
pub fn dealloc(mut addr: Paddr, mut size: usize) {
    let irq_guard = trap::disable_local();
    let local_pool_cell = LOCAL_POOL.get_with(&irq_guard);
    let mut local_pool = local_pool_cell.borrow_mut();
