...
pub fn execute_profile_command(_profile: &str, args: &ProfileArgs) {
    ...
    if let Some(parse_input) = &args.parse {
        let in_file = File::open(parse_input).expect("Failed to open input file");
        let profile: Profile =
            serde_json::from_reader(in_file).expect("Failed to parse the input JSON file");
        let out_file = File::create(out_args.output_path(Some(parse_input)))
...
impl Profile {
    fn serialize_to<W: Write>(&self, format: ProfileFormat, cpu_mask: u128, mut target: W) {
        match format {
            ...
            ProfileFormat::Json => {
                serde_json::to_writer(target, self).expect("Failed to write JSON output");
            }
        }
    }
}
