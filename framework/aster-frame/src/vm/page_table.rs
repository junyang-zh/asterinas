...
use crate::{
    arch::mm::{is_kernel_vaddr, is_user_vaddr, tlb_flush, PageTableEntry, ENTRY_COUNT_PER_PAGE},
    sync::SpinLock,
    vm::{config::PAGE_SIZE, VmFrame},
};
...
pub trait PageTableEntryTrait: Clone + Copy + Sized + Pod + Debug {
    ...
    ///
    /// For example, in x86 we use the following expression to get the index (ENTRY_COUNT_PER_PAGE is 512):
    /// ```
    /// va >> (12 + 9 * (level - 1)) & (ENTRY_COUNT_PER_PAGE - 1)
    /// ```
    ///
...
/// Read `ENTRY_COUNT_PER_PAGE` of PageTableEntry from an address
///
/// # Safety
///
...
pub unsafe fn table_of<'a, T: PageTableEntryTrait>(pa: Paddr) -> Option<&'a mut [T]> {
    ...
    Some(core::slice::from_raw_parts_mut(ptr, ENTRY_COUNT_PER_PAGE))
}
