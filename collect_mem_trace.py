...
def addr2func(frame_pc):
    global addr2func_cache

    if frame_pc in addr2func_cache:
        return addr2func_cache[frame_pc]

    # call addr2line to get the function name
    import subprocess
    p = subprocess.Popen(['addr2line', '-f', '-e', 'target/osdk/aster-nix/aster-nix-osdk-bin', '-a', hex(frame_pc)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    addr2func_cache[frame_pc] = processed
...
def main():
    ...
    for line in qemu_log:
        if "###DUMP_MEM_PROFILE_END###" in line:
            break
        if started:
            trace = line.strip()
        if "###DUMP_MEM_PROFILE_START###" in line:
            started = True
    import json
