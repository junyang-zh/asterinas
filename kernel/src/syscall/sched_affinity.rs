...
pub fn sys_sched_getaffinity(
    ...
    let cpu_set = match tid {
        0 => ctx.task.schedule_info().cpu_affinity.load(),
        _ => match thread_table::get_thread(tid) {
            Some(thread) => thread.task().schedule_info().cpu_affinity.load(),
...
pub fn sys_sched_setaffinity(
    ...
    match tid {
        0 => ctx.task.schedule_info().cpu_affinity.store(&user_cpu_set),
        _ => match thread_table::get_thread(tid) {
            Some(thread) => {
                thread
                    .task()
