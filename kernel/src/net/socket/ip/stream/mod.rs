...
impl StreamSocket {
    ...
    fn calc_io_events(&self) -> IoEvents {
        ...
        match state.as_ref() {
            State::Init(init_stream) => init_stream.calc_io_events(),
            State::Connecting(connecting_stream) => connecting_stream.calc_io_events(),
            State::Listen(listen_stream) => listen_stream.calc_io_events(),
            State::Connected(connected_stream) => connected_stream.calc_io_events(),
        }
    }
}
