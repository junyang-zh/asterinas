...
impl Vmar_ {
    fn alloc_growup_region(&self, size: usize, align: usize) -> Result<usize> {
        let base = self
            .alloc_base
            .fetch_update(Ordering::Release, Ordering::Acquire, |base| {
                Some(base.align_up(align) + size)
            })
            .unwrap();
        Ok(base)
    }

    fn maintain_alloc_start(&self, start: usize) {
        self.alloc_base.fetch_max(start, Ordering::AcqRel);
    }
    ...
    fn new_root() -> Arc<Self> {
        ...
        Arc::new(Vmar_ {
            ...
            alloc_base: AtomicUsize::new(ROOT_VMAR_GROWUP_BASE),
    ...
    fn clear_root_vmar(&self) -> Result<()> {
        ...
        self.alloc_base
            .store(ROOT_VMAR_GROWUP_BASE, Ordering::Release);
    ...
    pub(super) fn new_fork_root(self: &Arc<Self>) -> Result<Arc<Self>> {
        ...
        let new_alloc_base = self.alloc_base.load(Ordering::Acquire);
        ...
        Ok(Arc::new(Self {
            ...
            alloc_base: AtomicUsize::new(new_alloc_base),
    ...
    pub fn build(self) -> Result<Vaddr> {
        ...
        let map_to_addr = if can_overwrite {
            ...
            if offset + map_size > ROOT_VMAR_GROWUP_BASE {
                parent.0.maintain_alloc_start(offset + map_size);
            }
            offset
