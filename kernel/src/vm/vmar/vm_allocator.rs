...
mod simple_allocator {
    ...
    impl SimpleAllocator {
        pub fn new() -> Self {
            SimpleAllocator {
                free_range: SpinLock::new(ROOT_VMAR_LOWEST_ADDR << 8..INIT_STACK_CLEARANCE),
            }
        }
    }
    ...
    impl VmAllocator for SimpleAllocator {
        fn clear(&self) {
            *self.free_range.lock() = ROOT_VMAR_LOWEST_ADDR << 8..INIT_STACK_CLEARANCE;
        }
...
mod per_cpu_allocator {
    ...
    impl PerCpuAllocator {
        pub fn new() -> Self {
            ...
            let mut cur_base = ROOT_VMAR_LOWEST_ADDR << 8;
            let per_cpu_size = ((INIT_STACK_CLEARANCE - cur_base) / nr_cpus).align_down(PAGE_SIZE);
